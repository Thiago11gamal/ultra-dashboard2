...
export const KpiCard = React.memo(function KpiCard({ value, label, color, icon, sub }) {
    ...
    return (
        <div 
            className="relative flex flex-col justify-between rounded-3xl border border-white/5 bg-slate-900/40 backdrop-blur-xl group hover:border-white/10 hover:bg-slate-800/60 hover:-translate-y-1 transition-all duration-300 overflow-hidden"
            style={{ padding: '1.5rem', boxShadow: `0 8px 30px -15px ${color}30` }}
        >
            ...
            <div className="relative z-10">
                <p className="text-2xl sm:text-3xl md:text-[2rem] font-black tracking-tight min-w-0 truncate drop-shadow-md leading-tight pl-1.5" style={{ color }}>{value}</p>
                <p className="text-[10px] sm:text-xs text-slate-400 uppercase tracking-widest mt-2 font-bold group-hover:text-slate-300 transition-colors pl-1.5">{label}</p>
            </div>
        </div>
    );
});
