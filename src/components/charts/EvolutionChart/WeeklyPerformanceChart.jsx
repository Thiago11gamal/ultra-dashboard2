...
import { getDateKey, formatDisplayDate } from '../../../utils/dateHelper';
...
const WeeklyPerformanceChart = ({
    ...
    const chartData = React.useMemo(() => {
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const horas = Number((minutes / 60).toFixed(1));
            ...
            const acertos = questionsTotal > 0 
                ? Number(((correctTotal / questionsTotal) * maxScore).toFixed(1))
                : null;
                                        ...
                                        {payload.map((entry, index) => (
                                            <div key={index} className="flex items-center justify-between gap-4 py-1">
                                                ...
                                                <span className="text-xs font-black text-white">
                                                    {entry.value !== null ? `${entry.value}${entry.name === 'acertos' ? unit : 'h'}` : 'N/A'}
                                                </span>
                                            </div>
                                        ))}
                                    </div>
