...
export default function EvolutionChart({ categories = [], targetScore = 80 }) {
    ...
    const CustomTooltip = ({ active, payload, label, isCompare }) => {
        ...
        return (
            <div className="bg-slate-900/95 border border-slate-700 p-4 rounded-xl shadow-2xl text-sm min-w-[240px] z-50">
                <p className="text-slate-400 mb-3 font-bold border-b border-slate-700/80 pb-2">
                    📅 {label}
                </p>
                <div className="space-y-2">
                    {payload.map((p, i) => {
                        ...
                        return (
                            <div key={i} className="flex flex-col mb-2 bg-slate-800/50 p-2 rounded-lg border border-slate-700/30">
                                <div className="flex justify-between items-center mb-1">
                                    <span style={{ color: p.color }} className="font-bold text-xs uppercase tracking-wider">{p.name}</span>
                                    <span style={{ color: p.color }} className="font-bold">{Number(p.value).toFixed(1)}%</span>
                                </div>
                                {rawTotal && (
                                    <div className="text-[10px] text-slate-400 text-right">
                                        Acertos: <strong className="text-slate-200">{rawCorrect}</strong> / {rawTotal} q.
                                    </div>
                                )}
                            </div>
                        );
