                        ...
                        {ENGINES.map((eng) => {
                            ...
                            return (
                                <button
                                    ...
                                    aria-label={`Alternar para modo ${eng.label}`}
                                    aria-pressed={active}
                                    className={`shrink-0 group flex items-center justify-center gap-2 px-5 py-3 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/70 text-[10px] sm:text-[12px] font-black tracking-tight transition-all duration-300 border ${active
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 sm:gap-3 mb-3 sm:mb-5 w-full">
                    <div className="flex items-center justify-between gap-1 bg-slate-950/60 border border-slate-800/70 rounded-xl p-1 shrink-0 overflow-x-auto w-full sm:w-auto">
                        ...
                        {[{ label: '30d', value: '30' }, { label: '60d', value: '60' }, { label: '90d', value: '90' }, { label: 'Tudo', value: 'all' }].map(w => (
                            <button type="button" key={w.value} onClick={() => setTimeWindow(w.value)}
                                aria-label={`Filtrar período ${w.label}`}
                                aria-pressed={timeWindow === w.value}
                                className={`shrink-0 flex-1 sm:flex-none px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/70 text-[10px] sm:text-xs font-bold transition-all ${timeWindow === w.value ? 'bg-indigo-600/20 text-indigo-300 border border-indigo-600/40' : 'text-slate-500 hover:text-slate-300 border border-transparent'}`}>
                                {w.label}
                            </button>
                        ))}
                    </div>
                    ...
                    {activeEngine !== 'compare' && activeEngine !== 'mc_density' && (
                        <button type="button" onClick={() => setShowOnlyFocus(!showOnlyFocus)}
                            aria-label="Alternar filtro de foco"
                            aria-pressed={showOnlyFocus}
                            className={`shrink-0 flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/70 text-[10px] sm:text-xs font-bold border transition-all w-full sm:w-auto min-w-0 ${showOnlyFocus ? 'bg-amber-500/10 border-amber-500/40 text-amber-300' : 'bg-slate-900/40 border-slate-800 text-slate-500 hover:text-slate-300 hover:border-slate-700'}`}>
